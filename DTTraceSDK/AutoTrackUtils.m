...
+ (void)trackAppClickWithUITableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    @try {
        ...
        //忽略 $AppClick 事件
        if(![[SensorsAnalyticsSDK sharedInstance] isDtstackAutoTrackEventContains:DTSTACK_PAGE_CTL_ITEM_SELECTED]){
            return;
        }
        ...
        [properties setValue:@(DTSTACK_PAGE_CTL_ITEM_SELECTED) forKey:@"event_id"];
        [[SensorsAnalyticsSDK sharedInstance] track:@"$PAGE_CTL_ITEM_SELECTED" withProperties:properties];
    } @catch (NSException *exception) {
        SAError(@"%@ error: %@", self, exception);
    }
}
