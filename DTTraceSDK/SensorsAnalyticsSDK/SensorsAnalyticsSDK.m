...
// 自动追踪相关事件及属性
//dtstack 应用第一次启动
static NSString* const SYS_FIRST_START = @"$SYS_FIRST_START";
//dtstadck 应用启动
static NSString* const SYS_START = @"$SYS_START";
//dtstack 应用退出
static NSString* const SYS_END = @"$SYS_END";
//dtstack 应用恢复
static NSString* const SYS_RESTORE = @"$SYS_RESTORE";
//dtstack 应用从后台切换前台
static NSString* const SYS_FOREGROUND = @"$SYS_FOREGROUND";
// dtstack 应用进入后台
static NSString* const SYS_BACKGROUND = @"$SYS_BACKGROUND";
// dtstack 控件长按
static NSString* const PAGE_CTL_LONG_CLICKED = @"$PAGE_CTL_LONG_CLICKED";
// dtstack 控件点击
static NSString* const PAGE_CTL_CLICKED = @"$PAGE_CTL_CLICKED";
// dtstack 浏览页面
static NSString* const PAGE_ENTER = @"$PAGE_ENTER";
// dtstack 页面离开事件
static NSString* const PAGE_LEAVE = @"$PAGE_LEAVE";
//dtstack envetId
...
@property (atomic, copy) NSString *DTTID;
...
@implementation SensorsAnalyticsSDK {
    ...
    NSMutableArray *_dtstackAutoTrackEvent; // dtstack自动采集事件
}
...
- (instancetype)initWithServerURL:(NSString *)serverURL
    ...
    @try {
        if (self = [self init]) {
            ...
            //TODO 请求服务器来判断sdk是否开启
            //return nil;
            //TODO 请求服务器判断事件是否开启, 这里先写死
            _dtstackAutoTrackEvent = [[NSMutableArray alloc] init];
            NSArray *autoTrackEventArray = [NSArray arrayWithObjects:
            @(DTSTACK_SYS_FIRST_START),
            @(DTSTACK_SYS_START),
            @(DTSTACK_SYS_RESTORE),
            @(DTSTACK_SYS_BACKGROUND),
            @(DTSTACK_SYS_FOREGROUND),
            @(DTSTACK_SYS_END),
            @(DTSTACK_PAGE_ENTER),
            @(DTSTACK_PAGE_LEAVE),
            @(DTSTACK_PAGE_CTL_ITEM_SELECTED),
            @(DTSTACK_PAGE_CTL_CLICKED),
            @(DTSTACK_PAGE_CTL_LONG_CLICKED),
            nil];
            [self setDtstackAutoTrackEvent: autoTrackEventArray];
            
            NSDictionary *dataDict = [NSDictionary dictionaryWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"local" ofType:@"plist"]];
            NSMutableDictionary *mDict = [dataDict copy];
...
//            _flushBulkSize = 100;
            _flushInterval = [[dataDict valueForKey:@"flushInterval"] intValue];
            _flushBulkSize = [[dataDict valueForKey:@"flushBulkSize"] intValue];
...
- (NSDictionary *)getPresetProperties {
    ...
    @try {
        ...
        [properties setValue:[_automaticProperties objectForKey:@"$sdk_type"] forKey:@"$sdk_type"];
        [properties setValue:[_automaticProperties objectForKey:@"$sdk_version"] forKey:@"$sdk_version"];
        [properties setValue:@"Apple" forKey:@"$brand"];
        [properties setValue:_deviceModel forKey:@"$device_model"];
        ...
        [properties setObject:networkType forKey:@"$access"];
        if ([networkType isEqualToString:@"WIFI"]) {
            [properties setObject:@YES forKey:@"$access_subtype"];
        } else {
            [properties setObject:@NO forKey:@"$access_subtype"];
        }
...
- (void)setDtstackAutoTrackEvent:(NSArray*)eventTypes{
    for (id eventType in eventTypes) {
        if ([_dtstackAutoTrackEvent containsObject: eventType]) {
            continue;
        }
        [_dtstackAutoTrackEvent addObject: eventType];
    }
}

- (void)closeAllAutoTrackEvent{
    _dtstackAutoTrackEvent = [[NSMutableArray alloc] init];
    [_dtstackAutoTrackEvent addObject:@(DTSTACK_NONE)];
    _autoTrack = NO;
}

- (BOOL)isDtstackAutoTrackEventContains:(DTStackEventType)eventType{
    if ([_dtstackAutoTrackEvent containsObject:@(eventType)]) {
        return YES;
    }
    return NO;
...
- (void)trackFromH5WithEvent:(NSString *)eventInfo enableVerify:(BOOL)enableVerify {
    dispatch_async(self.serialQueue, ^{
        @try {
            ...
            [automaticPropertiesCopy removeObjectForKey:@"$sdk_type"];
            [automaticPropertiesCopy removeObjectForKey:@"$sdk_version"];
            ...
            if([type isEqualToString:@"track"] || [type isEqualToString:@"track_signup"]){
                ...
                // 每次 track 时手机网络状态
                NSString *networkType = [SensorsAnalyticsSDK getNetWorkStates];
                [propertiesDict setObject:networkType forKey:@"$access"];
                if ([networkType isEqualToString:@"WIFI"]) {
                    [propertiesDict setObject:@YES forKey:@"$access_subtype"];
                } else {
                    [propertiesDict setObject:@NO forKey:@"$access_subtype"];
                }
...
- (void)enableAutoTrack{
    ...
    dispatch_once(&onceToken, ^{
        if ([self isLaunchedPassively]) {
            // 追踪 AppStart 事件
            if ([self isDtstackAutoTrackEventContains:DTSTACK_SYS_START]) {
            ...
            if ([self isDtstackAutoTrackEventContains:DTSTACK_SYS_START]) {
                if (isFirstStart) {
                    [self track:SYS_FIRST_START withProperties:@{
                                                                 RESUME_FROM_BACKGROUND_PROPERTY : @(_appRelaunched),
                                                                 APP_FIRST_START_PROPERTY : @(isFirstStart),
                                                                 EVENT_ID: @(DTSTACK_SYS_FIRST_START),
                                                                 }];
                } else {
                    [self track:SYS_START withProperties:@{
                                                                 RESUME_FROM_BACKGROUND_PROPERTY : @(_appRelaunched),
                                                                 APP_FIRST_START_PROPERTY : @(isFirstStart),
                                                                 EVENT_ID: @(DTSTACK_SYS_START),
                                                                 }];
                }
            }
            // 启动 AppEnd 事件计时器
            if ([self isDtstackAutoTrackEventContains:DTSTACK_SYS_END]) {
                [self trackTimer:SYS_END withTimeUnit:SensorsAnalyticsTimeUnitSeconds];
            }
        }
    });
}
...
- (void)ignoreAutoTrackEventType:(NSArray *)eventTypes {
    for (NSNumber *eventId in eventTypes) {
        if ([_dtstackAutoTrackEvent containsObject:eventId]) {
            [_dtstackAutoTrackEvent removeObjectAtIndex:[_dtstackAutoTrackEvent indexOfObject:eventId]];
        }else {
            SADebug(@"_dtstackAutoTrackEvent don't contain this eventId: %@", eventId);
        }
    }
}
...
//        NSString *bodyLen = [NSString stringWithFormat:@"%u", [[postBody dataUsingEncoding:NSUTF8StringEncoding] length]];
//        [request addValue:bodyLen forHTTPHeaderField:@"Content-Length"];
        ...
        void (^block)(NSData*, NSURLResponse*, NSError*) = ^(NSData *data, NSURLResponse *response, NSError *error) {
            ...
            SADebug(@"========> accept the message %@", urlResponseContent);
            if(statusCode != 200) {
                messageDesc = @"\n【invalid message】\n";
                if (_debugMode != SensorsAnalyticsDebugOff) {
                    if (statusCode >= 300) {
...
- (void)enqueueWithType:(NSString *)type andEvent:(NSDictionary *)e {
    ...
    //添加请求次数
    [event setObject:@([self.messageQueue accumRequestAndGetCount]) forKey:@"$send_count"];
...
- (void)track:(NSString *)event withProperties:(NSDictionary *)propertieDict withType:(NSString *)type {
    ...
    [libProperties setValue:[_automaticProperties objectForKey:@"$sdk_type"] forKey:@"$sdk_type"];
    [libProperties setValue:[_automaticProperties objectForKey:@"$sdk_version"] forKey:@"$sdk_version"];
    ...
    if ([self isAutoTrackEnabled] && propertieDict) {
        if ([event isEqualToString:PAGE_CTL_CLICKED]) {
            if ([self isDtstackAutoTrackEventContains:DTSTACK_PAGE_CTL_CLICKED]) {
                lib_detail = [NSString stringWithFormat:@"%@######", [propertieDict objectForKey:@"$screen_name"]];
            }
        } else if ([event isEqualToString:PAGE_ENTER]) {
            if ([self isDtstackAutoTrackEventContains:DTSTACK_PAGE_ENTER]) {
                lib_detail = [NSString stringWithFormat:@"%@######", [propertieDict objectForKey:@"$screen_name"]];
            }
        }
    }
    ...
    if ([event isEqualToString:@"$SYS_END"]) {
        [self sendMessageByLibProperties:libProperties andProDict:propertieDict for:event and:type];
        if (_debugMode != SensorsAnalyticsDebugOff) {
            // 在DEBUG模式下，直接发送事件
            [self _flush:NO];
        } else {
...
- (void)sendMessageByLibProperties:(NSMutableDictionary *)libProperties andProDict:(NSDictionary *)propertieDict for:(NSString *)event and:(NSString *)type{
    NSNumber *currentSystemUpTime = @([[self class] getSystemUpTime]);
    NSNumber *timeStamp = @([[self class] getCurrentTime] / 1000);
    /**
     dtstack 修改, 设置当前发送的时间
     */
    ...
    if ([type isEqualToString:@"track"] || [type isEqualToString:@"track_signup"]) {
        ...
        // 每次 track 时手机网络状态
        NSString *networkType = [SensorsAnalyticsSDK getNetWorkStates];
        [p setObject:networkType forKey:@"$access"];
        if ([networkType isEqualToString:@"WIFI"]) {
            [p setObject:@YES forKey:@"$access_subtype"];
        } else {
            [p setObject:@NO forKey:@"$access_subtype"];
        }
    ...
    if (propertieDict) {
        ...
        [p setObject:timeStr forKey:@"$timestamp"];
        ...
        @try {
            ...
            if (self.locationConfig.enableGPSLocation) {
                if (CLLocationCoordinate2DIsValid(self.locationConfig.coordinate)) {
                    NSInteger latitude = self.locationConfig.coordinate.latitude;
                    NSInteger longitude = self.locationConfig.coordinate.longitude;
...
- (NSDictionary *)collectAutomaticProperties {
    ...
    [p setValue:[self DTTID] forKey:@"$DTTID"];
    [p addEntriesFromDictionary:@{
                                  @"$sdk_type": @"iOS",
                                  @"$sdk_version": [self libVersion],
                                  @"$brand": @"Apple",
                                  @"$os": @"iOS",
                                  ...
                                  @"$resolution": [NSString stringWithFormat:@"%ld x %ld", (NSInteger)size.height, (NSInteger)size.width],
                                  @"$token": @"token",
                                  @"$app_key": @"app_key"
                                      }];
    return [p copy];
...
- (void)unarchiveDTTID {
    NSString *archiveDTTID = (NSString *)[self unarchiveFromFile:[self filePathForData:@"DTTID"]];
    if (archiveDTTID == nil) {
        BOOL isReal;
        NSString *device_id = [[self class] getUniqueHardwareId:&isReal];
        int random = [MD5Encryption getRandomNumber:100000 to:1000000];
        UInt64 timeStamp = [[self class] getCurrentTime];
        self.DTTID = [MD5Encryption md5EncryptWithString:[NSString stringWithFormat:@"%@%d%llu", device_id, random, timeStamp]];
    } else {
        self.DTTID = archiveDTTID;
    }
    [self archiveDTTID];
}
...
- (void)archiveDTTID{
    NSString *filePath = [self filePathForData:@"DTTID"];
    /* 为filePath文件设置保护等级 */
    ...
    if (![NSKeyedArchiver archiveRootObject:[[self DTTID] copy] toFile:filePath]) {
        SAError(@"%@ unable to archive distinctId", self);
    }
    SADebug(@"%@ archived DTTID", self);
}
...
//    }
    ...
    if ([self isAutoTrackEnabled] && [self isDtstackAutoTrackEventContains:DTSTACK_PAGE_ENTER]) {
        [properties setValue:@(DTSTACK_PAGE_ENTER) forKey:EVENT_ID];
        [self track:PAGE_ENTER withProperties:properties];
    }
}
...
static void sa_imp_setJSResponderBlockNativeResponder(id obj, SEL cmd, id reactTag, BOOL blockNativeResponder){
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        @try {
            ...
            //忽略 $AppClick 事件
            if (![[SensorsAnalyticsSDK sharedInstance] isDtstackAutoTrackEventContains:DTSTACK_PAGE_CTL_CLICKED]) {
                return;
            }
...
- (void)_enableAutoTrack {
...
#endif
        //发送页面离开事件
        if ([self isAutoTrackEnabled] && [self isDtstackAutoTrackEventContains:DTSTACK_PAGE_LEAVE]) {
            if (![self shouldTrackClass: klass]) {
                return;
            }
            NSMutableDictionary *properties = [[NSMutableDictionary alloc] init];
            ...
            [properties setValue:@(DTSTACK_PAGE_LEAVE) forKey:EVENT_ID];
            [self track:PAGE_LEAVE withProperties:properties];
        }
    };
    ...
    [SASwizzler swizzleSelector:@selector(addGestureRecognizer:) onClass:[UIButton class] withBlock:gestureRecognizerAppClickBlock named:@"track_UIButton_addGestureRecognizer"];
    //$AppClick
...
- (void)trackGestureRecognizerAppClick:(id)target {
    @try {
        ...
        //忽略 app长按事件
        if (![self isDtstackAutoTrackEventContains:DTSTACK_PAGE_CTL_LONG_CLICKED]) {
            return;
        }
        ...
        if([gesture isKindOfClass:UILongPressGestureRecognizer.class]){
            //控件长按手势
            [properties setValue:@(DTSTACK_PAGE_CTL_LONG_CLICKED) forKey:EVENT_ID];
            [[SensorsAnalyticsSDK sharedInstance] track:PAGE_CTL_LONG_CLICKED withProperties:properties];
        } else {
            //TODO 其他手势
        }
...
- (void)trackViewScreen:(NSString *)url withProperties:(NSDictionary *)properties {
    ...
    [self track:PAGE_ENTER withProperties:trackProperties];
}
...
- (void)applicationWillEnterForeground:(NSNotification *)notification {
    ...
    _appRelaunched = YES;
    _launchOptions = nil;
    if ([self isDtstackAutoTrackEventContains:DTSTACK_SYS_FOREGROUND] && [self isAutoTrackEnabled]) {
        [self track:SYS_FOREGROUND withProperties:@{EVENT_ID: @(DTSTACK_SYS_FOREGROUND)}];
    }
}
...
- (void)applicationDidBecomeActive:(NSNotification *)notification {
    ...
    if ([self isAutoTrackEnabled] && _appRelaunched) {
        // 追踪 AppStart 事件
        if ([self isDtstackAutoTrackEventContains:DTSTACK_SYS_RESTORE]) {
            [self track:SYS_RESTORE withProperties:@{
                                                     RESUME_FROM_BACKGROUND_PROPERTY : @(_appRelaunched),
                                                         APP_FIRST_START_PROPERTY : @(isFirstStart),
                                                         EVENT_ID: @(DTSTACK_SYS_RESTORE),
                                                        }];
        }
        // 启动 应用进入后台 事件计时器
        if ([self isDtstackAutoTrackEventContains:DTSTACK_SYS_BACKGROUND]) {
            [self trackTimer:SYS_BACKGROUND withTimeUnit:SensorsAnalyticsTimeUnitSeconds];
        }
        // 启动应用退出事件计时器
//        if ([self isDtstackAutoTrackEventContains:DTSTACK_SYS_END]) {
//            [self trackTimer:SYS_END withTimeUnit:SensorsAnalyticsTimeUnitSeconds];
...
- (void)applicationDidEnterBackground:(NSNotification *)notification {
    ...
    if ([self isAutoTrackEnabled]) {
        ...
        if ([self isDtstackAutoTrackEventContains:DTSTACK_SYS_BACKGROUND]) {
            ...
            [self track:SYS_BACKGROUND withProperties:@{
                                                        EVENT_ID: @(DTSTACK_SYS_BACKGROUND)
                                                        }];
        }
    }
...
//            [self _flush:YES];
    ...
    SADebug(@"APP will terminal ~~~~~~~~~~");
    if ([self isAutoTrackEnabled]) {
        if ([self isDtstackAutoTrackEventContains:DTSTACK_SYS_END]) {
            [self track:SYS_END withProperties:@{
                                                 EVENT_ID: @(DTSTACK_SYS_END)
                                                 }];
        }
    }
}
